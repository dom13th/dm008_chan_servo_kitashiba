...
static int32_t	velocity;	// currently planned velocity (mostly unsigned)
static uint8_t	direction;	// 0 = positive, 1 = negative

static int32_t	start_pos;	// start position of the move
static int32_t	target_pos;	// target position of the move


/* --- private functions --------------------------------------------------- */

static void dg_0(void);
static void dg_1(void);
static uint8_t dg_add(void);
...
*/
	} while (Flags&0x30);	// 0b0110000  keep waiting if torque limit did occur
